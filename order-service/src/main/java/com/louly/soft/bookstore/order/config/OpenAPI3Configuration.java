package com.louly.soft.bookstore.order.config;
    ...
    OpenAPI openApi() {
        return new OpenAPI()
                .info(new Info()
                        ...
                        .version("v1.0.0")
                        .contact(new Contact().name("Louly-Soft").email("sulaynd@loulysoft.sn")))
                .servers(List.of(new Server().url(apiGatewayUrl)))
                .addSecurityItem(new SecurityRequirement().addList("Authorization"))
                .components(new Components()
                        .addSecuritySchemes(
                                // Add security scheme for OAuth2 Authorization Code Flow in Orders controllers also
                                "security_auth",
                                new SecurityScheme()
